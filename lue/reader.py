...
class Lue:
    def __init__(
        self, file_path, tts_model: TTSBase | None, overlap: float | None = None
    ):
    ...
    def _initialize_tts(self, tts_model):
        ...
        self.tts_voice = (
            tts_model.voice
            if tts_model and tts_model.voice
            else config.TTS_VOICES.get(tts_model.name)
            if tts_model
            else None
        )
        self._saved_word_highlight_mode = (
            None  # Stores previous mode before auto-disable
        )
    ...
    def _load_content(self, quiet=False):
        ...
        if not quiet:
            self.console.print(
                f"[bold cyan]Loading document: {self.book_title}...[/bold cyan]"
            )
        ...
        if not self.chapters or not any(chapter for chapter in self.chapters):
            self.console.print(
                f"[bold red]Error: No text could be extracted from the file.[/bold red]"
            )
            self.console.print(
                "This might happen with image-based PDFs or unsupported formats."
            )
            sys.exit(1)
    ...
    def _initialize_progress(self):
        ...
        self.chapter_idx, self.paragraph_idx, self.sentence_idx = (
            progress_manager.validate_and_set_progress(
                self.chapters, self.progress_file, c, p, s
            )
        )
    ...
    async def initialize_tts(self) -> bool:
        ...
        if not self.tts_model:
            self.console.print(
                "[yellow]No TTS model selected. TTS playback is disabled.[/yellow]"
            )
            return True
            ...
            if hasattr(self.tts_model, "supports_word_timing"):
                if not self.tts_model.supports_word_timing:
                    ...
                    if self._saved_word_highlight_mode is not None:
                        ...
                        self._saved_word_highlight_mode = (
                            None  # Clear saved mode after restore
                        )

            return True
        else:
            self.console.print(
                f"[bold red]Initialization of {self.tts_model.name.upper()} failed. TTS will be disabled.[/bold red]"
            )
            self.tts_model = None
            self.is_paused = True
            return False
    ...
    def _is_position_visible(self, chapter_idx, paragraph_idx, sentence_idx):
        ...
        if position_key not in self.position_to_line:
            return False
    ...
    def _is_position_near_current_reading(self, click_pos, threshold=3):
        ...
        try:
            current_chapter, current_paragraph, current_sentence = (
                self.chapter_idx,
                self.paragraph_idx,
                self.sentence_idx,
            )
            ...
            if abs(click_paragraph - current_paragraph) == 1:
                if click_paragraph < current_paragraph:  # Previous paragraph
                    ...
                    sentences_in_paragraph = len(
                        self.chapters[click_chapter][click_paragraph].split(". ")
                    )
    ...
    def _is_paragraph_near_current_reading(self, direction):
        ...
        try:
            ...
            if direction == "next":
                ...
                sentences_in_paragraph = len(
                    self.chapters[current_chapter][current_paragraph].split(". ")
                )
    ...
    def _scroll_to_position(
        self, chapter_idx, paragraph_idx, sentence_idx, smooth=True
    ):
        if not smooth and self._is_position_visible(
    ...
    def _smooth_scroll_to(self, target_offset, fast=False):
        self.target_scroll_offset = max(
            0, min(target_offset, len(self.document_lines) - 1)
        )
    ...
    async def _animate_scroll(self, fast=False):
        try:
            ...
            if abs(target_offset - start_offset) < 0.1:
                return
            (
                animation_speed,
                frame_delay,
                ...
                steps,
                max_steps,
            ) = 0.15, 0.03, 0.5, 2.0, 0, 80
            while (
                abs(self.scroll_offset - self.target_scroll_offset)
                > convergence_threshold
                and steps < max_steps
            ):
                ...
                max_scroll = max(
                    0, len(self.document_lines) - (ui.get_terminal_size()[1] - 4)
                )
            ...
            if (chap_idx, para_idx) in self.paragraph_line_ranges:
                ...
                for sent_idx, sentence in enumerate(sentences):
                    sentence_positions.append(
                        (current_char, current_char + len(sentence), sent_idx)
                    )
                    current_char += len(sentence) + 1
                wrapped_lines = Text(paragraph, justify="left", no_wrap=False).wrap(
                    self.console, max(20, width - 10)
                )
                line_offset = clicked_line - para_start
                if 0 <= line_offset < len(wrapped_lines):
                    char_pos_in_para = sum(
                        len(line.plain) for line in wrapped_lines[:line_offset]
                    ) + min(content_x, len(wrapped_lines[line_offset].plain))
    ...
    def _is_click_on_text(self, click_x, click_y):
        ...
        return (
            text_area_left <= click_x <= text_area_right
            and text_area_top <= click_y <= text_area_bottom
        )
    ...
    def _handle_progress_bar_click(self, click_x, click_y):
        ...
        if progress_bar_start_x <= click_x < progress_bar_start_x + progress_bar_width:
            ...
            self.scroll_offset = self.target_scroll_offset = max(
                0, min(target_scroll, max_scroll)
            )
    ...
    def _increase_speed(self):
        ...
        speed_levels = [
            round(i * 0.1, 1) for i in range(10, 31)
        ]  # 1.0 to 3.0 in 0.1 increments
        current_index = 0
    ...
    def _rewind_position(self, current_pos, mode="sentence"):
        ...
        while c >= 0:
            if p >= 0:
                ...
                if p >= 0:
                    s = (
                        len(content_parser.split_into_sentences(self.chapters[c][p]))
                        - 1
                    )
                else:
                    c -= 1
                    if c >= 0:
                        ...
                        s = (
                            len(
                                content_parser.split_into_sentences(self.chapters[c][p])
                            )
                            - 1
                        )
            else:
    ...
    def _calculate_progress_percentage(self):
        ...
        sentences_read = sum(
            ...
            for p in self.chapters[i]
        )
        if self.chapter_idx < len(self.chapters):
            sentences_read += sum(
                len(
                    content_parser.split_into_sentences(
                        self.chapters[self.chapter_idx][i]
                    )
                )
                for i in range(self.paragraph_idx)
            )
    ...
    def _scroll_to_position_immediate(self, chapter_idx, paragraph_idx, sentence_idx):
        if (chapter_idx, paragraph_idx, sentence_idx) in self.position_to_line:
            target_line = self.position_to_line[
                (chapter_idx, paragraph_idx, sentence_idx)
            ]
            _, height = ui.get_terminal_size()
            available_height = max(1, height - 4)
            new_offset = min(
                max(0, target_line - available_height // 2),
                max(0, len(self.document_lines) - available_height),
            )
            self.scroll_offset = self.target_scroll_offset = new_offset
    ...
    def _handle_scroll_down_immediate(self):
        ...
        self.scroll_offset = self.target_scroll_offset = min(
            max_scroll, self.scroll_offset + 1
        )
        if self.smooth_scroll_task and not self.smooth_scroll_task.done():
            self.smooth_scroll_task.cancel()
        self.chapter_idx, self.paragraph_idx, self.sentence_idx = (
    ...
    def _handle_navigation_immediate(self, cmd):
        ...
        new_pos = (
            self._advance_position(current_pos, mode)
            if direction == "next"
            else self._rewind_position(current_pos, mode)
        )
        if new_pos:
            self.first_sentence_jump = False
    ...
    def _handle_page_scroll_immediate(self, direction):
        ...
        new_offset = (
            max(0, self.scroll_offset - page_size)
            if direction < 0
            else min(
                max(0, len(self.document_lines) - page_size),
    ...
    def _handle_move_to_top_immediate(self):
        top_visible_line, bottom_visible_line = (
            int(self.scroll_offset),
            int(self.scroll_offset) + max(1, ui.get_terminal_size()[1] - 4),
        )
        ...
        if topmost_sentence:
            ...
            self.ui_chapter_idx, self.ui_paragraph_idx, self.ui_sentence_idx = (
                topmost_sentence
            )
    ...
    def _handle_move_to_top_smooth(self):
        ...
        self.auto_scroll_enabled = True
        if (
            config.SMOOTH_SCROLLING_ENABLED
            and topmost_sentence
            and topmost_sentence in self.position_to_line
        ):
    ...
    async def _handle_pause_toggle(self):
        ...
        async with self.pause_toggle_lock:
            ...
            if (
                self.current_pause_toggle_task
                and not self.current_pause_toggle_task.done()
            ):
    ...
    async def _ui_update_loop(self):
        last_update_time, last_sentence_pos, last_progress_save_time = (
            0,
            None,
            ...
            try:
                ...
                if not self.is_paused:
                    ...
                    if target_pos != (
                        ...
                        if (
                            self.first_sentence_jump
                            and last_sentence_pos is not None
                            and target_pos != last_sentence_pos
                        ):
                            self.first_sentence_jump = False
                        (
                            self.ui_chapter_idx,
    ...
    async def _word_update_loop(self):
        ...
        while self.running:
            try:
                if (
                    not self.is_paused
                    and hasattr(self, "current_sentence_words")
                    and hasattr(self, "current_sentence_duration")
                    and hasattr(self, "current_word_start_time")
                    and self.current_sentence_words
                    and self.current_sentence_duration > 0
                ):
                    elapsed = (
                        asyncio.get_event_loop().time() - self.current_word_start_time
                    )
                    ...
                    if total_words > 0:
                        ...
                        if (
                            hasattr(self, "current_word_timings")
                            and self.current_word_timings
                        ):
                            # Use word mapping if available to handle TTS word boundary mismatches
                            if (
                                hasattr(self, "current_word_mapping")
                                and self.current_word_mapping
                            ):
                                # Find which TTS word should be highlighted based on timing
                                tts_word_idx = None
                                tts_word_start = None
                                tts_word_end = None
                                for i, (word, start_time, end_time) in enumerate(
                                    self.current_word_timings
                                ):
                                    if (
                                        adjusted_elapsed >= start_time
                                        and adjusted_elapsed < end_time
                                    ):
                                        tts_word_idx = i
                                ...
                                if tts_word_idx is None and self.current_word_timings:
                                    sentence_duration = max(
                                        [end for _, _, end in self.current_word_timings]
                                    )
                                    if adjusted_elapsed >= sentence_duration:
                                        tts_word_idx = (
                                            len(self.current_word_timings) - 1
                                        )
                                        _, tts_word_start, tts_word_end = (
                                            self.current_word_timings[tts_word_idx]
                                        )
                                ...
                                if tts_word_idx is not None:
                                    ...
                                    mapped_orig_words = []
                                    for orig_idx, mapped_tts_idx in enumerate(
                                        self.current_word_mapping
                                    ):
                                    ...
                                    if mapped_orig_words:
                                        if len(mapped_orig_words) == 1:
                                            ...
                                            time_per_orig_word = tts_duration / len(
                                                mapped_orig_words
                                            )
                                            elapsed_in_tts_word = (
                                                adjusted_elapsed - tts_word_start
                                            )
                                            ...
                                            current_word_idx = mapped_orig_words[
                                                sub_word_idx
                                            ]
                                    else:
                                        # Fallback: use the TTS word index directly if no mapping found
                                        current_word_idx = min(
                                            tts_word_idx, total_words - 1
                                        )
                            else:
                                    ...
                                    if self.current_word_timings:
                                        ...
                                        sentence_duration = max(
                                            [
                                                ...
                                                for _, _, end in self.current_word_timings
                                            ]
                                        )
                                        if adjusted_elapsed >= sentence_duration:
                            ...
                            current_word_idx = min(
                                int(adjusted_elapsed / time_per_word), total_words - 1
                            )
    ...
    async def _shutdown(self):
        ...
        tasks_to_cancel = [
            self.smooth_scroll_task,
            self.ui_update_task,
            self.word_update_task,
            self.pending_restart_task,
            self.current_pause_toggle_task,
        ]
    ...
    def _handle_exit_signal(self, signum, frame):
        ...
        self.running = False
        if self.loop and self.loop.is_running():
            self.loop.call_soon_threadsafe(self._post_command_sync, "quit")
    ...
    async def run(self):
        ...
        if not self.chapters or not self.chapters[0]:
            return
        ...
        while self.running:
                ...
                if cmd in [
                    ...
                    if self.recent_books_list:
                        if self.recent_menu_selection_idx == 0:
                            self.recent_menu_selection_idx = (
                                len(self.recent_books_list) - 1
                            )
                        else:
                    ...
                    if (
                        ...
                        and 0
                        <= self.recent_menu_selection_idx
                        < len(self.recent_books_list)
                    ):
                        book_data = self.recent_books_list[
                            self.recent_menu_selection_idx
                        ]
                        path = book_data["path"]
            ...
            if isinstance(cmd, tuple):
                ...
                if command_name == "_update_highlight":
                    if not self.is_paused:
                        self.chapter_idx, self.paragraph_idx, self.sentence_idx = data
                    ...
                    self.current_sentence_duration = (
                        timing_info.get("speech_duration") or duration
                    )
                    ...
                    if clicked_position := self._find_sentence_at_click(*data):
                        ...
                        self.auto_scroll_enabled = False
                        self._save_extended_progress(sync_audio_position=True)
                        self.pending_restart_task = asyncio.create_task(
            ...
            if cmd == "finish":
                ...
                if not self.auto_scroll_enabled and self.resize_anchor:
                    ...
                    if anchor_pos in self.position_to_line:
                        ...
                        new_max_scroll = max(
                            0, len(self.document_lines) - new_available_height
                        )
                        new_scroll = target_line - int(
                            round(fraction_in_view * new_available_height)
                        )
                ...
                self.current_pause_toggle_task = asyncio.create_task(
                ...
                config.SENTENCE_HIGHLIGHTING_ENABLED = (
                    not config.SENTENCE_HIGHLIGHTING_ENABLED
                )
